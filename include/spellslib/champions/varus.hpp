...
public:
    VarusSpells(
        std::map<std::string, std::function<float()>> attributes = std::map<std::string, std::function<float()>>()) : ChampionSpells(attributes) {}
    ...
    Spell get_q() const override
    {
        ...
        attributes["range"] = [this, timeCast, gameTime]()
        {
            float time_cast = timeCast;
            float game_time = gameTime;
            float maxRange = find_property_from_map(this->create_map(), "maxRange");
            float minRange = find_property_from_map(this->create_map(), "minRange");
            return get_range(time_cast, game_time, maxRange, minRange);
        };

        return Spell(SpellType::LINEAR, attributes["range"](), speed, delay, attributes);
    }
