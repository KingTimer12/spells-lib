...
class ChampionSpells {
 ...
 protected:
  ...
  float find_property_from_map(
      const std::map<std::string, std::function<float()>>& map,
 ...
 public:
  ChampionSpells(std::map<std::string, std::function<float()>> attributes =
  ...
  template <typename T>
  void update_attributes(const std::string& field, T value) {
